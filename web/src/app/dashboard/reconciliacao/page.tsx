...
export default function ReconciliacaoPage() {
  const [relatorio, setRelatorio] = useState<RelatorioReconciliacao | null>(
    null,
  );
    ...
    <div className={styles.container}>
      ...
      <div className={styles.hero}>
        ...
        <p className={styles.heroDescription}>
          Acompanhe o status de reconciliação das parcelas processadas a partir
          dos arquivos de folha de pagamento importados.
        </p>
      </div>
              ...
              >
                {Object.entries(relatorio.data.byConsignataria).map(
                  ([consignataria, dados]) => (
                    ...
                    >
                      <h3 className={styles.consignatariaTitle}>
                        {consignataria}
                      </h3>
                      <div className={styles.consignatariaStats}>
                        <div className={styles.consignatariaStat}>
                          <span className={styles.consignatariaStatLabel}>
                            Total
                          </span>
                        ...
                        {Object.entries(dados.byStatus).map(
                          ([status, count]) => (
                            <div
                              key={status}
                              className={styles.consignatariaStat}
                            >
                              <span className={styles.consignatariaStatLabel}>
                                {getStatusLabel(status)}
                              </span>
