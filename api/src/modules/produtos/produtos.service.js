...
class ProdutosService {
    ...
    async listar(params) {
        ...
        const [total, items] = await Promise.all([
            database_1.prisma.produto.count({ where }),
            database_1.prisma.produto.findMany({
                where,
                skip,
                take: limit,
                orderBy: { nome: 'asc' },
    ...
    async buscarPorId(id) {
        const produto = await database_1.prisma.produto.findUnique({
            where: { id },
            include: {
                consignataria: true,
                margem: true,
    ...
    async criar(data) {
        ...
        const consignataria = await database_1.prisma.consignataria.findUnique({
        ...
        if (data.juros_minimo !== null && data.juros_maximo !== null) {
            ...
            if (!(0, validators_1.validarTaxas)(taxa_minima, taxa_maxima)) {
        ...
        if (data.prazo_minimo !== null && data.prazo_maximo !== null) {
            if (!(0, validators_1.validarPrazo)(data.prazo_minimo, 6, 240) || !(0, validators_1.validarPrazo)(data.prazo_maximo, 6, 240)) {
        ...
        return database_1.prisma.produto.create({
            data: {
                nome: data.nome,
                tipo: data.tipo,
    ...
    async atualizar(id, data) {
        const produto = await database_1.prisma.produto.findUnique({ where: { id } });
        if (!produto)
            throw new Error('Produto não encontrado');
        ...
        if (data.juros_minimo !== null || data.juros_maximo !== null) {
            ...
            if (taxa_minima && taxa_maxima && !(0, validators_1.validarTaxas)(taxa_minima, taxa_maxima)) {
                throw new Error('Taxa mínima deve ser menor ou igual à máxima (0.5% - 30% a.m.)');
            }
        }
        ...
        if (data.prazo_minimo !== null || data.prazo_maximo !== null) {
            ...
            if (prazo_min && prazo_max) {
                if (!(0, validators_1.validarPrazo)(prazo_min, 6, 240) || !(0, validators_1.validarPrazo)(prazo_max, 6, 240)) {
                    throw new Error('Prazo deve estar entre 6 e 240 meses');
                }
                if (prazo_min > prazo_max) {
                    throw new Error('Prazo mínimo não pode ser maior que prazo máximo');
        ...
        return database_1.prisma.produto.update({
            where: { id },
            data: {
