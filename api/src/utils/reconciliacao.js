...
exports.reconciliarParcelas = reconciliarParcelas;
const database_1 = require("../config/database");
function numeroParaFloat(value) {
...
async function reconciliarParcelas(linhas, arquivoIntegracaoId) {
    ...
    for (const linha of linhas) {
        ...
        const servidor = await database_1.prisma.servidor.findUnique({ where: { matricula: linha.servidor_matricula } });
        if (!servidor) {
            resultado.erros += 1;
        ...
        const parcela = await database_1.prisma.parcela.findFirst({
            where: {
        ...
        if (diff > tolerancia) {
            ...
            await database_1.prisma.parcela.update({
                where: { id: parcela.id },
                data: {
...
exports.default = reconciliarParcelas;
