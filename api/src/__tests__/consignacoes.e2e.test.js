...
describe('Consignacoes Module — E2E Tests', () => {
    ...
    let servidorId;
    let margemId;
    let consignatariaAId;
    let consignatariaBId;
    let produtoAId;
    let produtoBId;
    const contratosCriados = [];
    const randomDigits = (size) => {
        let value = '';
    ...
    beforeAll(async () => {
        ...
        const user = await database_1.prisma.usuario.create({
            data: {
        ...
        margemId = (await database_1.prisma.margem.create({
            data: {
        ...
        consignatariaAId = (await database_1.prisma.consignataria.create({
            data: {
        ...
        consignatariaBId = (await database_1.prisma.consignataria.create({
            data: {
        ...
        produtoAId = (await database_1.prisma.produto.create({
            data: {
        ...
        produtoBId = (await database_1.prisma.produto.create({
            data: {
        ...
        servidorId = (await database_1.prisma.servidor.create({
            data: {
        ...
        await database_1.prisma.margemServidor.create({
            data: {
    ...
    afterAll(async () => {
        if (contratosCriados.length > 0) {
            await database_1.prisma.parcela.deleteMany({ where: { contrato_id: { in: contratosCriados } } });
            await database_1.prisma.contrato.deleteMany({ where: { id: { in: contratosCriados } } });
        }
        if (servidorId && margemId) {
            await database_1.prisma.margemServidor.deleteMany({
                where: {
                    servidor_id: servidorId,
                    margem_id: margemId
                }
        ...
        if (servidorId) {
            await database_1.prisma.servidor.deleteMany({ where: { id: servidorId } });
        }
        if (produtoAId) {
            await database_1.prisma.produto.deleteMany({ where: { id: produtoAId } });
        }
        if (produtoBId) {
            await database_1.prisma.produto.deleteMany({ where: { id: produtoBId } });
        }
        if (consignatariaAId) {
            await database_1.prisma.consignataria.deleteMany({ where: { id: consignatariaAId } });
        }
        if (consignatariaBId) {
            await database_1.prisma.consignataria.deleteMany({ where: { id: consignatariaBId } });
        }
        if (margemId) {
            await database_1.prisma.margem.deleteMany({ where: { id: margemId } });
        }
        if (testUserId) {
            await database_1.prisma.logAuditoria.deleteMany({ where: { usuario_id: testUserId } });
            await database_1.prisma.usuario.deleteMany({ where: { id: testUserId } });
        }
        await app.close();
    ...
    it('E2E-CSN-03: should reject when margem is insuficiente', async () => {
        const margemExtra = await database_1.prisma.margem.create({
            data: {
        ...
        const produtoExtra = await database_1.prisma.produto.create({
            data: {
        ...
        await database_1.prisma.produto.deleteMany({ where: { id: produtoExtra.id } });
        await database_1.prisma.margem.deleteMany({ where: { id: margemExtra.id } });
    });
    it('E2E-CSN-04/05/06: should aprovar, ativar and generate parcelas', async () => {
        ...
        const aprovarResponse = await (0, supertest_1.default)(app.server)
        ...
        const ativarResponse = await (0, supertest_1.default)(app.server)
        ...
        const parcelasResponse = await (0, supertest_1.default)(app.server)
    ...
    it('E2E-CSN-07: should quitar consignacao ativa', async () => {
        ...
        await (0, supertest_1.default)(app.server)
            ...
            .set('Authorization', `Bearer ${token}`)
            .send();
        const quitarResponse = await (0, supertest_1.default)(app.server)
    ...
    it('E2E-CSN-E01: should reject invalid transition QUITADA -> ATIVA', async () => {
        ...
        await (0, supertest_1.default)(app.server)
            ...
            .set('Authorization', `Bearer ${token}`)
            .send();
        const invalidResponse = await (0, supertest_1.default)(app.server)
            .put(`/v1/consignacoes/${consignacaoId}/ativar`)
            .set('Authorization', `Bearer ${token}`)
            .send();
    ...
    it('E2E-CSN-08: should cancelar consignacao pendente', async () => {
        ...
        const consignacaoId = createResponse.body.id;
        contratosCriados.push(consignacaoId);
        const cancelResponse = await (0, supertest_1.default)(app.server)
    ...
    it('E2E-CSN-10: should create portabilidade from consignacao ativa', async () => {
        ...
        await (0, supertest_1.default)(app.server)
            ...
            .set('Authorization', `Bearer ${token}`)
            .send();
        const portResponse = await (0, supertest_1.default)(app.server)
